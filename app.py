...
st.set_page_config(
    ...
    page_icon="📰", 
    layout="wide",  # Changed to wide for a modern dashboard feel
    initial_sidebar_state="collapsed"
)

# Custom minimal CSS to polish buttons and margins
st.markdown("""
    <style>
...
# 3. Your exact current explicit string headers mapping
topic_mapping = {
...
with col1:
    ...
    user_article = st.text_area(
        ...
        label_visibility="collapsed" # Hides the redundant label for a cleaner UI
    )
    ...
    if analyze_button:
        if user_article.strip() == "":
            ...
            with st.spinner("Processing text and mapping cluster space..."):
                ...
                # Beautiful, high-contrast results display box
                st.markdown(f"""
                ...
                for idx, (cid, name) in enumerate(topic_mapping.items()):
                    with metrics_cols[idx]:
                        # Shorten name for metric layout
                        short_name = name.split(" ")[0]
                        st.metric(label=short_name, value=scores[cid])
    else:
        # Placeholder view when the app is resting
